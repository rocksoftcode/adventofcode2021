class Cuboid {
	...
	def split(Cuboid cuttingCuboid) {
		...
		List<BigInteger> middleXRange = [max(this.minX, cuttingCuboid.minX), min(this.maxX, cuttingCuboid.maxX)];
		...
		List<BigInteger> middleYRange = [max(this.minY, cuttingCuboid.minY), min(this.maxY, cuttingCuboid.maxY)];
